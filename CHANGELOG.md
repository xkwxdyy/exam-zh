...
- 调整既有 CTAN 标签的发布校验：核验标签中已提交的两份手册 PDF，并继续在隔离环境中完成示例、回归、归档和上传验证，避免旧标签因字体不可用而无法复现手册。

## [0.3.6] - 2026-08-01

### Fixed

- 扩展 Dashboard 的 AI 发布前准备步骤，在整理 changelog 的同时归档新增测试文件、排除测试生成物，并按用户可见改动的需要同步完整手册和入门手册。
- 修复隐藏答案时 `fillin/width=auto` 将自然宽度填空压成刚性空白，导致 `\fillin*` 的换行位置与显示答案时不一致的问题；现保留答案内容自身的 CJK 与数学断点，同时隐藏文字并保留下划线，并使以中文开头的可换行答案在行末至少保留两个汉字。

## [0.3.5] - 2026-07-31

### Fixed

- 修复发布链续跑成功后仍显示旧失败步骤和继续按钮的问题，以同一版本的最新成功任务终结旧 checkpoint 并显示完成状态。
- 修复发布参数区域的桌面端网格留白，将跳过编译开关并入参数行，并在窄屏上保持整行布局。
- 发布链完整成功后，Dashboard 下次打开会自动切换到下一补丁版本、当天日期和匹配的提交信息，同时继续保留未完成任务的原参数以供恢复。
- 修复隐藏答案时 `fillin/width=auto` 的短填空与后续标点在行尾可能拒绝换行并越出版心的问题（GitHub #52）。
- 修复 `choices/margin` 在多行选项中仅缩进首行的问题，使后续选项行使用相同的左右边距（GitHub #47）。
- 稳定 XeTeX 下 `linegoal` 的横坐标记录，避免 `fillin/width=auto` 因 1sp 级位置波动导致 `latexmk` 多轮编译无法收敛（GitHub #51）。
- 修复 GitHub #30 原始复现中 `choices` 的常规内联 `\verb` 选项无法编译的问题。

## [0.3.4] - 2026-07-30

### Fixed

- 修复发布控制台失败后重复执行的问题：持久保存与目标版本绑定的步骤进度，重新打开时恢复原版本并从失败步骤继续，同时在缺少未发布变更片段时提前停止。

## [0.3.3] - 2026-07-30

### Changed

- 重构发布控制台：Changelog 改为手动生成，后续步骤按纵向发布链自动执行，并支持 GitHub/Gitee、全部发布与单独 CTAN 三种出口。
