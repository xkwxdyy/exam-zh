(() => {
  ...
  const state = { workflows: [], status: null, token: "", filter: "changes", activeJob: null, cursor: 0, pollTimer: null };
  ...
  function renderWorkflows() {
    const filtered = state.workflows.filter((item) => state.filter === "all" || item.stage === state.filter);
  ...
  async function confirmDialogRun() {
    ...
    if ($("#confirm-input").value.trim() !== expected) { toast(`确认词不匹配，应为 ${expected}`, "error"); return; }
    closeDialog();
